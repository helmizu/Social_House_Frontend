...
class AbstractChart extends Component {
  ...
  renderVerticalLabels = config => {
    const { labels = [], width, height, paddingRight, paddingTop, horizontalOffset = 10 } = config
    const fontSize = 12
    return labels.map((label, i) => {
      return (
        <Text
          key={Math.random()}
          x={((width + 24) / labels.length * (i)) + paddingRight + horizontalOffset}
  ...
  renderVerticalDataLabels = config => {
    const { labels = [], width, height, paddingRight, paddingTop, horizontalOffset = 0, data } = config
    const fontSize = 12
    return labels.map((label, i) => {
      return (
        <Text
          key={Math.random()}
          x={((width + 50) / labels.length * (i)) + paddingRight + horizontalOffset}
          y={((height / 4 * 3 * (1 - ((data[i] - Math.min(...data)) / this.calcScaler(data)))) + paddingTop - 8)}
          fontSize={fontSize}
