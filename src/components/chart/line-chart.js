...
class LineChart extends AbstractChart {
    ...
    data.map((dataset,index)=>{
      dataset.data.map((x, i) => {
        output.push (
          <Circle
            key={Math.random()}
            cx={paddingRight + (i * (width + (50)) / dataset.data.length)}
      ...
      output.push (
        <Polygon
          ...
          points={dataset.data.map((x, i) =>
            (paddingRight + (i * (width + (50)) / dataset.data.length)) +
          ',' +
  ...
  getBezierLinePoints = (dataset, config) => {
    ...
    const x = i => Math.floor(paddingRight + i * (width + (50)) / dataset.data.length)
  ...
  renderBezierShadow = config => {
    ...
    data.map((dataset,index)=>{
      ...
      ` L${paddingRight + ((width + (50)) / dataset.data.length * (dataset.data.length - 1))},${(height / 4 * 3) + paddingTop} L${paddingRight},${(height / 4 * 3) + paddingTop} Z`
      output.push (
        <Path
          key={index}
          d={d}
